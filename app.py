...
CREATE TABLE IF NOT EXISTS stories (
    ...
    city TEXT,
    theme TEXT,
    dimmed TEXT NOT NULL,
    reclaimed TEXT NOT NULL
)
""")
conn.commit()

# Helper to load all stories
def load_stories():
    return pd.read_sql("SELECT * FROM stories ORDER BY id DESC", conn)
...
with tab_submit:
    ...
    st.markdown("This is a prototype. Submissions are safely stored in a database.")
    ...
    with st.form("submission_form", clear_on_submit=True):
        ...
        if submitted:
            if dimmed.strip() == "" or reclaimed.strip() == "":
                ...
                c.execute("""
                    INSERT INTO stories (title, city, theme, dimmed, reclaimed)
                    VALUES (?, ?, ?, ?, ?)
